...
Requires Playwright; serve the built site under /PORTAL/ to test Pages subpaths.
"""
import functools
import http.server
import json
...
with tempfile.TemporaryDirectory() as temp:
    ...
    with sync_playwright() as p:
        ...
        assert page.locator('#project-count').inner_text() == '03 projects'
        ...
        assert page.locator('.project-card h3').inner_text() == 'PianoRules'
        ...
        assert page.locator('h1').inner_text() == 'Privacy & credits.'
        context.close()
        ...
        page.goto(url)
        page.wait_for_selector('.project-card')
        assert page.locator('.project-card h3').inner_text() == '<img onerror=alert(1)>'
